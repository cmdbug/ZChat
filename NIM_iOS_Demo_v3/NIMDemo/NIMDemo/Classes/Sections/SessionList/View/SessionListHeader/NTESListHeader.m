...
- (void)refreshWithType:(NTESListHeaderType)type value:(id)value{
    switch (type) {
        case ListHeaderTypeCommonText://普通
            [self refreshWithCommonText:value];
            break;
        case ListHeaderTypeNetStauts://网络状态
            [self refreshWithNetStatus:[value integerValue]];
            break;
        case ListHeaderTypeLoginClients://多端登录
...
- (void)refreshWithClients:(NSArray *)clients{
    ...
    if (clients.count) {
        ...
        text = name.length? [NSString stringWithFormat:@"正在使用藤信%@版",name] : @"正在使用藤信未知版本";
    }
