...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //wzteng增加图标Icon
    if (tableRow.rowIcon != nil) {
//        cell.imageView.image = [UIImage imageNamed:tableRow.rowIcon];
        UIImage *image = [UIImage imageNamed:tableRow.rowIcon];
        cell.imageView.image = image;
        float scale = 0.0f;
        float size = 25.0f;
        size = tableRow.uiRowHeight <= size ? tableRow.uiRowHeight - 2 : size;
        CGSize itemSize = CGSizeMake(0, 0);
        if (image.size.width < image.size.height) {
            scale = image.size.width / image.size.height;
            itemSize = CGSizeMake(size, size / scale);
        } else {
